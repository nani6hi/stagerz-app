...
| Repo-defined CI | **On `main`: none.** On branch `phase-21.8a-s8-deletion-drainer`, **one manual-only workflow is prepared but not yet committed and has never run** — `.github/workflows/process-pending-asset-deletions.yml` (Phase 21.8A). Trigger is **`workflow_dispatch` only**: **no schedule**, and no `push` or `pull_request` trigger, so it adds no PR checks. `permissions: {}`, no checkout, no retries. It requires the repository secret `STAGERZ_MAINTENANCE_SECRET`, which **does not exist yet**, and fails before any request without it. **No Supabase mutation has occurred.** The only PR checks remain Netlify's |
