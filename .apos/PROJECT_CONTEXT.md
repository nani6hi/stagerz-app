...
| Current phase status | **PAUSED for the Phase 21.5, 21.6 and 21.7 security detours — 21.5 and 21.6 merged, 21.7 in preparation. Step 1 complete; extraction still in progress.** Phase 21.3's Step 1 discovered finding **S-1** (unauthenticated write path to `public.users` via the `public_profiles` view). The phase was **paused**, S-1 was remediated separately as **Phase 21.4 / PR #11** (merge `968b501`, now on `main` at `f4d1fa7`), and the paused Phase 21.3 artefacts — lost from volatile scratch storage — were **recovered hash-verified** from Claude file-history plus a deterministic re-run of the original generators (all 10 artefacts matched their recorded SHA-256). It then paused a second time for **Phase 21.5**, which remediated **S-2** (PR #13), and a third time for **Phase 21.6**, which remediated the root cause **S-5**. **Security findings, live-verified 2026-09-11: S-1 REMEDIATED (21.4); S-2 REMEDIATED (21.5); S-5 REMEDIATED (21.6); S-3, S-4, S-6, S-7 remain OPEN** — `storage.objects` still has no DELETE policy; **55** of 58 backend SQLSTATEs unhandled (corrected from 54 — `23505` is constraint-raised, not one of the 58); `anon`/`authenticated` still hold `MAINTAIN` on `public.users` (S-6); three trigger functions are still executable by `PUBLIC` (S-7). **S-5, the root cause of S-1 and S-2, is closed for future objects**: the `postgres` default privileges in `public` and `storage` no longer grant anything to `anon` or `authenticated`. One latent twin remains outside this project's control — the `supabase_admin`/`public` TABLES default. Contract detail below is the historical Step 1 record, with post-21.4 and post-21.5 state marked separately |
| Phase 21.7 status | **PREPARATION ONLY — NOT APPROVED, NOT APPLIED.** Branch `phase-21.7-s6-s7-privilege-cleanup` from `main` @ `452b12b`. Targets the two residues found during the Phase 21.6 analysis: **S-6** (`anon`/`authenticated` hold `MAINTAIN` on `public.users`) and **S-7** (three `log_collaboration_*_activity` trigger functions executable by `PUBLIC`, and explicitly by `authenticated`). Root cause of S-6 traced exactly: five enumerated revokes across four migrations on 2026-07-12, written against the pre-PostgreSQL-17 set of seven privileges — the arithmetic leaves precisely `m`, and no migration mentions `MAINTAIN` anywhere. S-7 comes from PostgreSQL's built-in `EXECUTE`-to-`PUBLIC` default, not from S-5. Both are very low / low severity: `MAINTAIN` grants no row access and no HTTP verb reaches a maintenance command, and a direct call to a trigger function raises `0A000` before its body runs. Prepared migration is **exactly 7 statements** — 1 `REVOKE MAINTAIN` for S-6, 6 `REVOKE EXECUTE` for S-7 — no `GRANT`/`ALTER`/`DROP`/`CREATE`/`CASCADE`. The `authenticated` revoke is **approved scope expansion** beyond S-7's original PUBLIC-only wording, so the three functions end in the same `postgres`/`service_role`-only posture as `handle_new_auth_user()`, which is also the in-project proof that trigger firing does not require the caller to hold EXECUTE. Level 1; `index.html` untouched. Output: `analysis/phase-21.7/` |
...
| Repo-defined CI | **None.** No `.github/workflows` exists. The only PR checks are Netlify's |
