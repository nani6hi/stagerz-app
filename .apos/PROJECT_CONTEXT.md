...
| Current phase status | **RESUMED 2026-08-31 after the Phase 21.4 security detour. Step 1 complete; extraction still in progress.** Phase 21.3's Step 1 discovered finding **S-1** (unauthenticated write path to `public.users` via the `public_profiles` view). The phase was **paused**, S-1 was remediated separately as **Phase 21.4 / PR #11** (merge `968b501`, now on `main` at `f4d1fa7`), and the paused Phase 21.3 artefacts — lost from volatile scratch storage — were **recovered hash-verified** from Claude file-history plus a deterministic re-run of the original generators (all 10 artefacts matched their recorded SHA-256). **Security findings, live-verified 2026-08-31: S-1 REMEDIATED; S-2, S-3, S-4, S-5 remain OPEN** — `_test_results`/`_test_run_log` still world-writable by `anon` (RLS off, 0 policies, TRUNCATE granted); `storage.objects` still has no DELETE policy; **55** of 58 backend SQLSTATEs unhandled (corrected from 54 — `23505` is constraint-raised, not one of the 58); `pg_default_acl` still grants ALL on new TABLES to `anon`. Contract detail below is the historical Step 1 record, with post-21.4 state marked separately |
