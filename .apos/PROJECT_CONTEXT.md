...
| 21.3 | Capture the Backend Contract in the Repository | **Current phase of record — PAUSED, NOT COMPLETE.** Resumed 2026-08-31 after the 21.4 detour, then paused again for the 21.5–21.9 finding detours, all now closed; it may be resumed as a separate step. Addresses 20.7 item **C-3**. Step 1 (client-side audit + extraction planning) complete; live read-only extraction performed (19 tables, 1 view, 34 functions, 27 RLS policies, 2 storage policies, 4 triggers, 5 realtime tables). Discovered **S-1** (remediated by 21.4) plus **S-2–S-5**, which were still open at the 2026-08-31 resumption. **As of 2026-09-15, S-1 through S-8 are all REMEDIATED.** Paused artefacts were lost from volatile storage and **recovered hash-verified**. Remaining work: transcribe verbatim function bodies into the SQL snapshots. **No backend write has ever been performed by this phase.** `analysis/phase-21.3/` |
