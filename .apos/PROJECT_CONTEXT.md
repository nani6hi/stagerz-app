...
| Current development branch | **None active (2026-09-17).** Work is on `main` after the backend integrity remediation (PR #21). The Phase 21.3 resumption branch has not been created yet; it will be a fresh branch from `main`. *Historical:* `phase-21.3-backend-contract` was originally branched from `main` at `ebfe536` and fast-forwarded to `f4d1fa7` after PR #11. Its tip `2895367` was merged in PR #12 and it is retained, but it is based on pre-remediation state and is not to be reused |
...
| 21.3 | Capture the Backend Contract in the Repository | **Current phase of record — READY TO RESUME, INCOMPLETE (2026-09-17).** Resumed 2026-08-31 after the 21.4 detour, then paused again for the 21.5–21.9 finding detours and, on 2026-09-15, for the O-1/O-2/O-3 backend integrity remediation. All of these are now closed; resuming is a separate, approved step. Addresses 20.7 item **C-3**. Step 1 (client-side audit + extraction planning) complete; live read-only extraction performed (19 tables, 1 view, 34 functions, 27 RLS policies, 2 storage policies, 4 triggers, 5 realtime tables). Discovered **S-1** (remediated by 21.4) plus **S-2–S-5**, which were still open at the 2026-08-31 resumption. **As of 2026-09-15, S-1 through S-8 are all REMEDIATED.** Paused artefacts were lost from volatile storage and **recovered hash-verified**. Remaining work, re-derived 2026-09-15: populate all six SQL snapshots from the current post-remediation backend (none of them has per-object detail yet; 0 of 34 function bodies captured) and run R-1–R-11 / S-1–S-6. **No backend write has ever been performed by this phase.** `analysis/phase-21.3/` |
