...
| Current phase status | **PAUSED AGAIN 2026-09-08 for the Phase 21.5 security detour, now COMPLETE. Step 1 complete; extraction still in progress.** Phase 21.3's Step 1 discovered finding **S-1** (unauthenticated write path to `public.users` via the `public_profiles` view). The phase was **paused**, S-1 was remediated separately as **Phase 21.4 / PR #11** (merge `968b501`, now on `main` at `f4d1fa7`), and the paused Phase 21.3 artefacts — lost from volatile scratch storage — were **recovered hash-verified** from Claude file-history plus a deterministic re-run of the original generators (all 10 artefacts matched their recorded SHA-256). It then paused a second time for **Phase 21.5**, which remediated **S-2**. **Security findings, live-verified 2026-09-08: S-1 REMEDIATED (21.4); S-2 REMEDIATED (21.5); S-3, S-4, S-5 remain OPEN** — `storage.objects` still has no DELETE policy; **55** of 58 backend SQLSTATEs unhandled (corrected from 54 — `23505` is constraint-raised, not one of the 58); `pg_default_acl` still grants ALL on new TABLES to `anon`. **S-5 is the root cause of both fixed findings and is untouched** — the next table created by `postgres` in `public` will reproduce the same exposure. Contract detail below is the historical Step 1 record, with post-21.4 and post-21.5 state marked separately |
| Phase 21.5 status | **COMPLETE — S-2 REMEDIATED. Not yet committed, not yet pushed, no PR.** Dropped `public._test_results` and `public._test_run_log` on **2026-09-08** under explicit approval. Both were abandoned test-harness tables: created outside all 41 migrations, written once each on the project's creation day, RLS disabled with 0 policies while `anon` and `authenticated` held all seven privileges **including TRUNCATE** — grants applied automatically by S-5, never deliberately. **Zero dependencies of every class and zero `index.html` references**, verified live twice. The 28 rows — a structured 24-case backend behaviour suite with real documentary value — were preserved in `analysis/phase-21.5/pre-drop-snapshot.sql`, **committed (`d5417e9`) and pushed to `origin` before the mutation ran**, deliberately avoiding the single-failure-domain mistake that lost the Phase 21.3 artefacts. All **17 pre-flight requirements matched exactly**; exactly two bare `DROP TABLE` statements executed (no `CASCADE`, no `IF EXISTS` — their success is itself proof nothing depended on them). Post-state: `public` tables **19 → 17**, both owned sequences gone, relation list lost exactly those four objects, views/functions/policies/triggers/default-ACLs/migration-records all unchanged. Advisor **ERROR findings 3 → 1**; the `rls_disabled_in_public` lint no longer appears; **zero new findings**. As `anon`, both REST endpoints now return **HTTP 404 `PGRST205`** while `public_profiles` still returns 200. Production smoke passed at both levels — server-side automated, and manually confirmed in a browser afterwards (Stage loads, navigation visible, a profile opens, no visible errors); the browser half was held open as NOT RUN until actually observed rather than inferred. Level 1; `index.html` untouched. Output: `analysis/phase-21.5/` |
