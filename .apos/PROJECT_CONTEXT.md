...
| Current development branch | **`phase-21.3-backend-contract-resume` (local only, not pushed, 2026-09-18).** Branched from `main` @ `2fdef81` for the Phase 21.3 resumption; five commits: `e5244a9` (snapshot at the pre-R-5 epoch), `b0c0a49` and `4e687ee` (R-5 remediation prepared and finalised), `cd2e95c` (apply and behavioural revalidation evidence) and `42afd8b` (snapshot regenerated after R-5). Pushing, PR and merge each need their own approval; a merge to `main` is a production release. *Historical:* `phase-21.3-backend-contract` was originally branched from `main` at `ebfe536` and fast-forwarded to `f4d1fa7` after PR #11. Its tip `2895367` was merged in PR #12 and it is retained, but it is based on pre-remediation state and is not to be reused |
| Previous development branches | `phase-21.2-startup-resilience` — merged in PR #9 (`968b501`), **retained**; `phase-21.2-docs-followup` — merged in PR #10 (`ebfe536`); `phase-21.4-s1-anon-write-exposure` — merged in PR #11 (`f4d1fa7`); `phase-21.3-backend-contract` — merged in PR #12 (`4ed53b9`), retained; `backend-integrity-o1-o2-o3` — merged in PR #21 (`e7fd4ac`), retained |
