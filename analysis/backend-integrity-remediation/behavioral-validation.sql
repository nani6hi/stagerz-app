...
begin
  ...
  begin
    ...
    results := results || 'T1 FAIL direct application insert succeeded';
  ...
  begin
    ...
    results := results || 'T3 FAIL owner DELETE did not raise';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
    results := results || case when v_state = '42501' and v_msg like 'permission denied%'
  ...
  begin
    delete from public.wanted_posts where id = p_post2;
    results := results || 'T4 FAIL privileged DELETE cascaded / succeeded';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate;
  ...
  begin
    ...
    v_step := 'delete';
    delete from public.wanted_posts where id = p_post2;
    results := results || 'T5 FAIL former owner DELETE did not raise';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate;
  ...
  begin
    ...
    results := results || 'T7 PASS frontend-shaped INSERT succeeded';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate;
    v_asset := null;
  ...
  begin
    ...
    results := results || 'T8 FAIL explicit id accepted';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
    results := results || case when v_state = '42501' and v_msg like 'permission denied%'
  ...
  begin
    ...
    insert into public.collaboration_assets
      ...
      values (now() - interval '1 year', p_collab, p_o_user,
              p_collab::text || '/validation-' || gen_random_uuid()::text, 'v', 'text/plain', 1, 'document');
    results := results || 'T9 FAIL explicit created_at accepted';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
    results := results || case when v_state = '42501' and v_msg like 'permission denied%'
  ...
  begin
    ...
    insert into public.collaboration_assets
      ...
      values (now(), p_collab, p_o_user,
              p_collab::text || '/validation-' || gen_random_uuid()::text, 'v', 'text/plain', 1, 'document');
    results := results || 'T10 FAIL explicit deleted_at accepted';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
    results := results || case when v_state = '42501' and v_msg like 'permission denied%'
  ...
  begin
    ...
    results := results || 'T11 FAIL mismatched path prefix accepted';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
  ...
  begin
    ...
    results := results || 'T11b FAIL folder-only path accepted';
  exception when others then
    get stacked diagnostics v_state = returned_sqlstate, v_msg = message_text;
    results := results || case when v_state = '42501' and v_msg like '%row-level security%'
  ...
  if v_asset is null then
    results := results || 'T12 SKIPPED (T7 did not create a row)';
  else
    begin
      perform set_config('request.jwt.claims', json_build_object('sub', p_o_auth, 'role', 'authenticated')::text, true);
      perform set_config('request.jwt.claim.sub', p_o_auth::text, true);
