...
| **S-4** | Unhandled backend SQLSTATEs — raw backend error text reaches the user | ⚠️ **OPEN — remediation implemented, reviewed and locally validated; branch pushed, PR #20 open; not merged or production-validated (Phase 21.9, branch `phase-21.9-s4-error-contract`, commit `ab3a46d`).** Re-derived 2026-09-14: **121** RAISE sites, **58** custom codes (**53** client-facing, **5** admin-only); the historical "55 unhandled" overstated the gap because every RPC site already had a fallback. The real flaw was that the fallback **displayed** the raw token, constraint or JSON: **34** reachable codes, about 12 user-facing conditions. The fix is a frontend-only translation layer (no backend change) covering all 20 RPC fallbacks, the 5 direct-write fallbacks, the Storage and Auth display sites, and a zero-byte upload guard. Independent pre-commit review completed (two corrections applied). Static check **41/41**, offline browser harness **55/55**, and real-application Level 3 local browser validation **50/50** with network isolation (0 successful external requests, 0 Supabase requests). Committed as `ab3a46d`, pushed, **PR #20 open** against `main` (GitHub PR review and checks in progress). **Outstanding:** complete the PR review and checks, merge PR #20 if approved, any specifically approved release or test-project validation, and final S-4 closure. Detail: `analysis/phase-21.9/` |
...
**S-1, S-2, S-3, S-5, S-6 and S-7 are now remediated, each by its own approved phase — S-1 by Phase 21.4, S-2 by Phase 21.5, S-5 by Phase 21.6, S-6 and S-7 by Phase 21.7, S-3 by Phase 21.8B. S-4 remains OPEN; its Phase 21.9 remediation is implemented, independently reviewed, locally validated in the real application, committed and pushed with PR #20 open, but not yet merged or production-validated.**
...
> **Phase 21.9 note (2026-09-14) — locally validated, not yet merged.** This section describes the frontend at `ebfe536`. On branch `phase-21.9-s4-error-contract` (commit `ab3a46d`, pushed; PR #20 open, not yet merged), every backend failure is routed through a single translator (`backendErrorMessage()` in `index.html`). The four handlers below keep their behaviour, and the full code-to-category map is recorded in `analysis/phase-21.9/`. This section is unchanged until that work is merged.
